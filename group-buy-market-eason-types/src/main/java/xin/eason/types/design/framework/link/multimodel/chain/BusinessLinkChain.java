 ...
 * @param <D> 动态上下文
 */
public class BusinessLinkChain<T, R, D> extends LinkChain<ILogicChainNodeHandler<T, R, D>> implements ILogicChainNodeHandler<T, R, D>{
    ...
    public R apply(T requestParam, D dynamicContext) {
        // 将 当前 指针指向第一个元素
        Node<ILogicChainNodeHandler<T, R, D>> current = firstNode;
        // 使用 do while 遍历所有责任链的节点
        do {
            // 取出节点内容, 执行其中的逻辑
            ILogicChainNodeHandler<T, R, D> data = current.data;
